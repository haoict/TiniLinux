...
void SettingsManager::set(const std::string& key, const std::string& value) { this->Settings[key] = value; }
...
void SettingsManager::save_to_file(const std::string& Path) {
    ...
    for (const auto& [key, value] : this->Settings) {
        if (!key.empty() && !value.empty()) Out << key << "=" << value << std::endl;
    }
    Out.close();
}
...
void SettingsManager::load_from_file(const std::string& Path) {
    ...
    while (std::getline(In, Line)) {
        ...
        if (!key.empty() && !value.empty()) Settings[key] = value;
    }
