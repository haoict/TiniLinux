...
SDL_Keycode VirtualKeyboard::keys[2][NUM_ROWS][NUM_KEYS] = {{{SDLK_ESCAPE, SDLK_F1, SDLK_F2, SDLK_F3, SDLK_F4, SDLK_F5, SDLK_F6, SDLK_F7, SDLK_F8, SDLK_F9, SDLK_F10, SDLK_F11, SDLK_F12},
...
const char* VirtualKeyboard::syms[2][NUM_ROWS][NUM_KEYS] = {{{"Esc", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "F10", "F11", "F12", nullptr},
...
VirtualKeyboard::~VirtualKeyboard() { cleanup(); }
...
SDL_Texture* VirtualKeyboard::get_cached_bitmap_texture(char c, SDL_Color color) {
    ...
    if (cached_color.r != color.r || cached_color.g != color.g || cached_color.b != color.b || cached_color.a != color.a) {
        clear_texture_cache();
        cached_color = color;
    }
...
SDL_Texture* VirtualKeyboard::get_cached_ttf_texture(char c, SDL_Color color, SDL_Renderer* renderer) {
    ...
    // Check if we need to clear cache due to color change
    if (cached_ttf_color.r != color.r || cached_ttf_color.g != color.g || cached_ttf_color.b != color.b || cached_ttf_color.a != color.a) {
        clear_ttf_texture_cache();
...
void VirtualKeyboard::pre_cache_common_chars(SDL_Color color, SDL_Renderer* renderer) {
    ...
    if (bitmap_font) {
        ...
        const char* key_labels[] = {"Esc",   "F1",   "F2",  "F3",    "F4",    "F5",  "F6",  "F7",  "F8",   "F9",  "F10", "F11",  "F12",   "Tab",   "Caps",
                                    "Shift", "Ctrl", "Alt", "Space", "Enter", "Bsp", "Ins", "Del", "Home", "End", "PrS", "Exit", "Pg Up", "Pg Dn", nullptr};
        ...
        switch (key) {
            ...
            case SDLK_RETURN: {
                ...
                if (strncmp(syms[shifted][selected_j][selected_i], "Exit", 4) == 0) {
                    ...
                    update_shift_state();
                } else if (target_key == SDLK_LCTRL || target_key == SDLK_RCTRL || target_key == SDLK_LALT || target_key == SDLK_RALT || target_key == SDLK_LGUI || target_key == SDLK_RGUI || target_key == SDLK_CAPSLOCK) {
